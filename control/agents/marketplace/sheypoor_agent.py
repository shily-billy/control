"""Sheypoor Agent - Manages Sheypoor marketplace"""
...
class SheypoorAgent(MarketplaceAgent):
    """Agent for managing Sheypoor marketplace (شیپور)"""

    def __init__(self, credentials: Optional[Dict[str, Any]] = None, config: Optional[Dict[str, Any]] = None):
        super().__init__("SheypoorAgent", credentials, config)
    ...
    async def authenticate(self) -> bool:
        try:
            if not self.credentials.get('username') or not self.credentials.get('password'):
                logger.error("Sheypoor requires username and password")
                return False
            logger.info(f"Authenticating with Sheypoor as {self.credentials.get('username')}")
    ...
    async def start(self):
        try:
            logger.info("Starting Sheypoor agent")
    ...
    async def stop(self):
        try:
            logger.info("Stopping Sheypoor agent")
            self.status = AgentStatus.IDLE
        except Exception as e:
    ...
    async def post_product(self, product: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            product_id = f"sheypoor_{len(self.listings) + 1}"
            ...
            logger.info(f"Product posted to Sheypoor: {product_id}")
            return {
                "success": True,
    ...
    async def update_product(self, product_id: str, product: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"Product updated on Sheypoor: {product_id}")
    ...
    async def delete_product(self, product_id: str) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"Product deleted from Sheypoor: {product_id}")
            return {"success": True, "product_id": product_id}
        except Exception as e:
