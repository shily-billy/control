"""Manamod Agent - Manages Manamod affiliate store"""
...
class ManamodAgent(AffiliateAgent):
    """Agent for managing Manamod affiliate (مناموج)"""

    def __init__(self, credentials: Optional[Dict[str, Any]] = None, config: Optional[Dict[str, Any]] = None):
        super().__init__("ManamodAgent", credentials, config)
        self.base_url = "https://manamod.net/partner"
    ...
    async def authenticate(self) -> bool:
        try:
            if not self.credentials.get('partner_id') or not self.credentials.get('api_key'):
                logger.error("Manamod requires partner_id and api_key")
                return False
            logger.info(f"Authenticating with Manamod partner {self.credentials.get('partner_id')}")
    ...
    async def start(self):
        try:
            logger.info("Starting Manamod agent")
    ...
    async def stop(self):
        try:
            logger.info("Stopping Manamod agent")
            self.status = AgentStatus.IDLE
        except Exception as e:
