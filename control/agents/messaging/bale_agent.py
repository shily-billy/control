"""Bale Agent - Handles Bale messaging"""
...
class BaleAgent(MessagingAgent):
    """Agent for managing Bale messaging (بله)"""

    def __init__(self, credentials: Optional[Dict[str, Any]] = None, config: Optional[Dict[str, Any]] = None):
        super().__init__("BaleAgent", credentials, config)
        self.user_id = credentials.get('user_id') if credentials else None
        self.messages: Dict[str, List[Dict[str, Any]]] = {}

    async def authenticate(self) -> bool:
        try:
            if not self.user_id:
                logger.error("Bale requires user_id")
                return False
            logger.info(f"Authenticating with Bale: {self.user_id}")
    ...
    async def start(self):
        try:
            logger.info("Starting Bale agent")
    ...
    async def stop(self):
        try:
            logger.info("Stopping Bale agent")
            self.status = AgentStatus.IDLE
        except Exception as e:
    ...
    async def send_message(self, chat_id: str, message: str) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"Bale message sent to {chat_id}")
    ...
    async def handle_message(self, message: Dict[str, Any]) -> str:
        try:
            logger.info("Handling Bale message")
            return "سلام!"
        except Exception as e:
